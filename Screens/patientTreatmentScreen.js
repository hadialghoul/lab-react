...
export default function PatientTreatmentScreen() {
  ...
  const fetchPatients = async (showRefreshLoader = false) => {
    ...
    try {
      ...
      console.log('=== DEBUGGING PATIENT FETCH ===');
      ...
      // Use the patients endpoint which works for doctors
      console.log('Fetching patients for logged-in doctor...');
      const res = await fetchWithAuth(`${Config.BASE_URL}/accounts/patients/`, { method: 'GET' });

      // debug: log status for troubleshooting
      console.log('fetchPatients status:', res.status);
